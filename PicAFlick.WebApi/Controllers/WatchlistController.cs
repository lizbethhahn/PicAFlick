...
namespace PicAFlick.WebApi.Controllers
{
    ...
    public class WatchlistController(IWatchlistService watchlistService, IHostEnvironment env) : ControllerBase
    {
        ...
        {
            var items = await _watchlistService.GetAllAsync(ct);
            return Ok(items);
        }
        ...
        {
            var created = await _watchlistService.AddAsync(dto, ct);
            if (created != null)
            {
        ...
        public async Task<IActionResult> Delete(int id, CancellationToken ct)
        {
            ...
            await _watchlistService.RemoveFromWatchlistAsync(id, ct);
            return NoContent();
        }
